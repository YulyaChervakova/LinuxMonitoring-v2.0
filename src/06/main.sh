...
sudo goaccess -p /usr/local/etc/goaccess /home/tameikah/linux/src/04/log*.log "$additional" --date-format=%d/%b/%Y --log-format='%h %^ %^ [%d:%t %^] \"%r\" %s %b %^ %u' --time-format=%T -o /home/tameikah/linux/src/06/report4.html
# sudo goaccess ../04/log*.log "$additional" --date-format=%d/%b/%Y --log-format='%h %^ %^ [%d:%t %^] \"%r\" %s %b %^ %u' --time-format=%T -o /home/tameikah/linux/src/06/report.html
